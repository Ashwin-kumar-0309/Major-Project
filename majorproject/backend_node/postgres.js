...
client.connect()
    .then(() => {
        ...
        console.log(`📊 Connected to database: ${process.env.DB_NAME || 'myprojectdb'}`);
        return ensureConsumerProductsTable();
    })
    .catch((err) => {
        console.error('❌ Database connection failed:', err.message);
        console.error('\n⚠️  IMPORTANT: PostgreSQL Connection Error!');
        console.error('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
        console.error('To fix this, follow these steps:\n');
        console.error('OPTION 1 - Update Password in .env file:');
        console.error('   1. Open: majorproject/backend_node/.env');
        console.error('   2. Update DB_PASSWORD to your PostgreSQL password\n');
        console.error('OPTION 2 - Reset PostgreSQL Password:');
        console.error('   1. Open pgAdmin or psql');
        console.error('   2. Run: ALTER USER postgres PASSWORD \'jay@662\';\n');
        console.error('OPTION 3 - Create Database:');
        console.error('   1. Run: psql -U postgres');
        console.error('   2. Run: CREATE DATABASE myprojectdb;');
        console.error('   3. Run the SQL from: database/setup_database.sql\n');
        console.error('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
        console.error('⚠️  Server will continue but database features won\'t work!\n');
        process.exit(1); // Exit to force user to fix the issue
    });
...
async function addProduct(serialNumber, name, brand, manufacturer, image, expiryDate){
    try {
        const result = await client.query('INSERT INTO product (serialNumber, name, brand, manufacturer_username, image, expiry_date) VALUES ($1, $2, $3, $4, $5, $6)', 
...
app.get('/company-products/:username', async (req, res)=>{
    ...
    try {
        ...
        const result = await client.query(
            'SELECT * FROM product WHERE manufacturer_username = $1 ORDER BY serialnumber DESC', 
            [username]
        );
...
app.listen(port, ()=>{
    console.log('\n🚀 ════════════════════════════════════════════════════');
    console.log(`   Identeefi Backend Server Started Successfully!`);
    console.log('   ════════════════════════════════════════════════════');
    console.log(`   📡 Server URL: http://localhost:${port}`);
    console.log(`   📊 Database: ${process.env.DB_NAME || 'myprojectdb'}`);
